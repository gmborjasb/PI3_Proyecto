...
function QuickStat({
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-slate-400 mt-1 mb-3 line-clamp-1">{app.university}</p>
          <Clock size={12} />
          {app.daysLeft > 0 ? `Cierra en ${app.daysLeft}d` : "Cerrada"}
        </div>
        <div className="w-7 h-7 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/30 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
...
function GoalCard({ goal }: { goal: (typeof mockGoals)[0] }) {
  ...
  const bgMap: Record<string, string> = {
    ...
    violet: "bg-violet-50 dark:bg-violet-950/20 border-violet-100 dark:border-violet-900/40",
  };
  ...
  return (
    <div className={cn("rounded-2xl border p-5", bgMap[goal.color])}>
      <div className="flex items-center gap-3 mb-4">
        <span className="text-2xl">{goal.emoji}</span>
        <div>
          <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm">{goal.name}</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {goal.progress} de {goal.total} pasos
          </p>
        </div>
        <span className={cn("ml-auto text-sm font-bold", textMap[goal.color])}>{percent}%</span>
      </div>
      <div className="w-full bg-white/60 dark:bg-slate-800/60 rounded-full h-2.5">
        <div
          className={cn("rounded-full h-2.5 transition-all duration-700", colorMap[goal.color])}
          style={{ width: `${percent}%` }}
        />
      </div>
    </div>
...
function TimelineStep({
  ...
  return (
    <div className="flex gap-4 items-start relative z-10">
      <div
        className={cn(
          "w-5 h-5 rounded-full flex items-center justify-center border-2 bg-white dark:bg-slate-900 mt-0.5 flex-shrink-0",
          status === "completed"
            ? "border-emerald-500"
            : status === "current"
              ? "border-blue-500"
      ...
      >
        {status === "completed" && <CheckCircle2 size={12} className="text-emerald-500" />}
        {status === "current" && <div className="w-2 h-2 rounded-full bg-blue-500" />}
      </div>
      <div>
        <p
          className={cn(
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span
        className={cn(
          "text-sm flex-1",
          met ? "text-slate-700 dark:text-slate-300" : "text-slate-900 dark:text-white font-medium"
        )}
      >
        {label}
      </span>
      {!met && (
        <span className="text-xs text-amber-600 dark:text-amber-400 font-semibold">Pendiente</span>
      )}
    </div>
  );
}
